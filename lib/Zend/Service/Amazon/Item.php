...
class Zend_Service_Amazon_Item
{
    ...
    /**
     * @var Zend_Service_Amazon_UserReview[]
     */
    public $UserReviews = array();
    ...
    public function __construct($dom)
    {
        ...
        $result = $xpath->query('./az:UserReviews/az:Review', $dom);
        if ($result->length >= 1) {
            /**
             * @see Zend_Service_Amazon_UserReview
             */
            #require_once 'Zend/Service/Amazon/UserReview.php';
            foreach ($result as $review) {
                $this->UserReviews[] = new Zend_Service_Amazon_UserReview($review);
            }
            $this->AverageRating = (float) $xpath->query('./az:UserReviews/az:AverageRating/text()', $dom)->item(0)->data;
            $this->TotalReviews = (int) $xpath->query('./az:UserReviews/az:TotalReviews/text()', $dom)->item(0)->data;
        }
