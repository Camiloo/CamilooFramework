...
{
    const XML_PATH_DEFAULT_CREATEACCOUNT_GROUP = 'user/create_account/default_group';
    ...
    public function getQuote()
    {
        if (is_null($this->_quote)) {
            ...
            elseif($this->getStoreId() && $this->hasUserId()) {
                $this->_quote->setStoreId($this->getStoreId())
                    ->setUserGroupId(Mage::getStoreConfig(self::XML_PATH_DEFAULT_CREATEACCOUNT_GROUP))
                    ->assignUser($this->getUser())
                    ->setIsActive(false)
                    ->save();
     ...
     * To enable quick switch of preconfigured user
...
/**
     ...
     */
    public function getUser($forceReload=false, $useSetStore=false)
    {
        if (is_null($this->_user) || $forceReload) {
            $this->_user = Mage::getModel('user/user');
            if ($useSetStore && $this->getStore()->getId()) {
                $this->_user->setStore($this->getStore());
            }
            if ($userId = $this->getUserId()) {
                $this->_user->load($userId);
            }
        }
        return $this->_user;
    }
