 ...
 * @method Mage_Newsletter_Model_Subscriber setUserId(int $value)
...
class Mage_Newsletter_Model_Subscriber extends Mage_Core_Model_Abstract
{
    ...
    /**
     * Load subscriber info by user
     *
     * @param Mage_User_Model_User $user
     * @return Mage_Newsletter_Model_Subscriber
     */
    public function loadByUser(Mage_User_Model_User $user)
    {
        $data = $this->getResource()->loadByUser($user);
        $this->addData($data);
        if (!empty($data) && $user->getId() && !$this->getUserId()) {
            $this->setUserId($user->getId());
            $this->setSubscriberConfirmCode($this->randomSequence());
            if ($this->getStatus()==self::STATUS_NOT_ACTIVE) {
                $this->setStatus($user->getIsSubscribed() ? self::STATUS_SUBSCRIBED : self::STATUS_UNSUBSCRIBED);
            }
            $this->save();
        }
        return $this;
    ...
    public function subscribe($email)
    {
        ...
        $userSession = Mage::getSingleton('user/session');
        ...
        if (!$this->getId() || $this->getStatus() == self::STATUS_UNSUBSCRIBED || $this->getStatus() == self::STATUS_NOT_ACTIVE) {
            if ($isConfirmNeed === true) {
                ...
                $ownerId = Mage::getModel('user/user')
                    ->setWebsiteId(Mage::app()->getStore()->getWebsiteId())
                    ->loadByEmail($email)
                    ->getId();
                $isOwnSubscribes = ($userSession->isLoggedIn() && $ownerId == $userSession->getId());
        ...
        if ($userSession->isLoggedIn()) {
            $this->setStoreId($userSession->getUser()->getStoreId());
            $this->setUserId($userSession->getUserId());
        } else {
    ...
    /**
     * Saving user subscription status
     *
     * @param   Mage_User_Model_User $user
     * @return  Mage_Newsletter_Model_Subscriber
     */
    public function subscribeUser($user)
    {
        ...
        if ($user->getImportMode()) {
            $this->setImportMode(true);
        }

        if (!$user->getIsSubscribed() && !$this->getId()) {
            // If subscription flag not set or user is not a subscriber
       ...
       /*
        * Logical mismatch between user registration confirmation code and user password confirmation
        */
       $confirmation = null;
       if ($user->isConfirmationRequired() && ($user->getConfirmation() != $user->getPassword())) {
           $confirmation = $user->getConfirmation();
       }

        $sendInformationEmail = false;
        if ($user->hasIsSubscribed()) {
            $status = $user->getIsSubscribed() ? (!is_null($confirmation) ? self::STATUS_UNCONFIRMED : self::STATUS_SUBSCRIBED) : self::STATUS_UNSUBSCRIBED;
            /**
             * If subscription status has been changed then send email to the user
             */
        ...
        if(!$this->getId()) {
            $this->setStoreId($user->getStoreId())
        ...
        $this->save();
        $sendSubscription = $user->getData('sendSubscription') || $sendInformationEmail;
    ...
    {
        $name = null;
        if ($this->hasUserFirstname() || $this->hasUserLastname()) {
