...
$emailAttributeId = $installer->getAttributeId('user', 'email');
$groupAttributeId = $installer->getAttributeId('user', 'group_id');

$installer->run("
    UPDATE {$this->getTable('user_entity')} user, {$this->getTable('core_store')} store
    SET user.website_id=store.website_id
    WHERE store.store_id=user.store_id;

    UPDATE {$this->getTable('core_website')} SET code='admin', name='Admin' WHERE website_id=0;

    UPDATE {$this->getTable('user_entity')} user, {$this->getTable('user_entity_varchar')} varchar_attribute
    SET user.email=varchar_attribute.value
    WHERE varchar_attribute.entity_id=user.entity_id
    AND varchar_attribute.attribute_id={$emailAttributeId};

    UPDATE {$this->getTable('user_entity')} user, {$this->getTable('user_entity_int')} int_attribute
    SET user.group_id=int_attribute.value
    WHERE int_attribute.entity_id=user.entity_id
    AND int_attribute.attribute_id={$groupAttributeId};
");

$installer->getConnection()->dropColumn($this->getTable('user_entity'), 'parent_id');
