...
-- DROP TABLE IF EXISTS `{$installer->getTable('user/eav_attribute')}`;
CREATE TABLE `{$installer->getTable('user/eav_attribute')}` (
...
$select = $setup->select()
    ...
    ->where('path=?', 'user/address/prefix_show')
    ->where('value!=?', '0');
$showPrefix = (bool)Mage::helper('user/address')->getConfig('prefix_show')
    || $setup->fetchOne($select) > 0;

$select = $setup->select()
    ->from($installer->getTable('core/config_data'), 'COUNT(*)')
    ->where('path=?', 'user/address/middlename_show')
    ->where('value!=?', '0');
$showMiddlename = (bool)Mage::helper('user/address')->getConfig('middlename_show')
    || $setup->fetchOne($select) > 0;

$select = $setup->select()
    ->from($installer->getTable('core/config_data'), 'COUNT(*)')
    ->where('path=?', 'user/address/suffix_show')
    ->where('value!=?', '0');
$showSuffix = (bool)Mage::helper('user/address')->getConfig('suffix_show')
    || $setup->fetchOne($select) > 0;

$select = $setup->select()
    ->from($installer->getTable('core/config_data'), 'COUNT(*)')
    ->where('path=?', 'user/address/dob_show')
    ->where('value!=?', '0');
$showDob = (bool)Mage::helper('user/address')->getConfig('dob_show')
    || $setup->fetchOne($select) > 0;

$select = $setup->select()
    ->from($installer->getTable('core/config_data'), 'COUNT(*)')
    ->where('path=?', 'user/address/taxvat_show')
    ->where('value!=?', '0');
$showTaxVat = (bool)Mage::helper('user/address')->getConfig('taxvat_show')
    || $setup->fetchOne($select) > 0;
...
$setup->insert($installer->getTable('eav/form_type'), array(
    'code'      => 'user_account_create',
    'label'     => 'user_account_create',
    'is_system' => 1,
    'theme'     => '',
    'store_id'  => 0
));
$formTypeId   = $setup->lastInsertId();
$entityTypeId = $installer->getEntityTypeId('user');
...
$setup->insert($installer->getTable('eav/form_type'), array(
    'code'      => 'user_account_edit',
    'label'     => 'user_account_edit',
    'is_system' => 1,
    'theme'     => '',
    'store_id'  => 0
));
...
$setup->insert($installer->getTable('eav/form_type'), array(
    'code'      => 'user_address_edit',
    'label'     => 'user_address_edit',
    'is_system' => 1,
    'theme'     => '',
    'store_id'  => 0
));
$formTypeId   = $setup->lastInsertId();
$entityTypeId = $installer->getEntityTypeId('user_address');
