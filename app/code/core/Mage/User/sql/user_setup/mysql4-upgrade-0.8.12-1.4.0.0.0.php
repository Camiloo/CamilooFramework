...
$installer->updateEntityType('user', 'additional_attribute_table', 'user/eav_attribute');
$installer->updateEntityType('user', 'entity_attribute_collection', 'user/attribute_collection');
$installer->updateEntityType('user_address', 'additional_attribute_table', 'user/eav_attribute');
$installer->updateEntityType('user_address', 'entity_attribute_collection', 'user/address_attribute_collection');
$installer->run("
...
$stmt = $installer->getConnection()->select()
    ...
    ->where('entity_type_id = ?', $installer->getEntityTypeId('user'))
    ->orWhere('entity_type_id = ?', $installer->getEntityTypeId('user_address'));
$result = $installer->getConnection()->fetchAll($stmt);

$table = $installer->getTable('user/eav_attribute');
