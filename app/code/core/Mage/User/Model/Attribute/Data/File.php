...
/**
 * User Attribute File Data Model
 *
 * @category    Mage
 * @package     Mage_User
 * @author      Camilooframework Core Team <core@magentocommerce.com>
 */
class Mage_User_Model_Attribute_Data_File extends Mage_User_Model_Attribute_Data_Abstract
{
    /**
    ...
    protected function _validateByRules($value)
    {
        ...
        if (!empty($rules['file_extensions'])) {
            ...
            if (!in_array($extension, $extensions)) {
                return array(
                    Mage::helper('user')->__('"%s" is not a valid file extension.', $label)
                );
            }
        }
        ...
        if (!empty($rules['max_file_size'])) {
            ...
            if ($rules['max_file_size'] < $size) {
                return array(
                    Mage::helper('user')->__('"%s" exceeds the allowed file size.', $label)
                );
            };
        }
    ...
    public function validateValue($value)
    {
        ...
        if (is_array($value)) {
            ...
            $filePath = Mage::getBaseDir('media') . DS . 'user' . $value;
     ...
     * @param array|string $value
     * @return Mage_User_Model_Attribute_Data_File
     */
    ...
    public function outputValue($format = Mage_User_Model_Attribute_Data::OUTPUT_FORMAT_TEXT)
    {
        ...
        if ($value) {
            switch ($format) {
                case Mage_User_Model_Attribute_Data::OUTPUT_FORMAT_JSON:
                    $output = array(
                        'value'     => $value,
