...
class Mage_User_Model_Convert_Adapter_User
    extends Mage_Eav_Model_Convert_Adapter_Entity
{
    ...
    /**
     * Retrieve user group collection array
     *
     * @return array
     */
    public function getUserGroups()
    {
        if (is_null($this->_userGroups)) {
            ...
            $collection = Mage::getModel('user/group')
                ->getCollection()
                ->addFieldToFilter('user_group_id', array('gt'=> 0));
            foreach ($collection as $group) {
                $this->_userGroups[$group->getUserGroupCode()] = $group->getId();
            }
        }
    ...
    /**
     * Alias at getUserGroups()
     *
     * @return array
     */
    public function getUserGoups()
    {
    ...
    {
        $this->setVar('entity_type', 'user/user');

        if (!Mage::registry('Object_Cache_User')) {
            $this->setUser(Mage::getModel('user/user'));
        }
        //$this->setAddress(Mage::getModel('catalog/'))

        foreach (Mage::getConfig()->getFieldset('user_dataflow', 'admin') as $code=>$node) {
            if ($node->is('ignore')) {
    ...
    public function load()
    {
        ...
        $attrFilterArray ['user_address/telephone'] = array(
            'type'  => 'like',
            'bind'  => $addressType
        );
        $attrFilterArray ['user_address/postcode']  = array(
            'type'  => 'like',
            'bind'  => $addressType
        );
        $attrFilterArray ['user_address/country']   = array(
            'type'  => 'eq',
            'bind'  => $addressType
        );
        $attrFilterArray ['user_address/region']    = array(
            'type'  => 'like',
            'bind'  => $addressType
        );
        ...
        $attrToDb = array(
            'group'                     => 'group_id',
            'user_address/country'  => 'user_address/country_id',
        );
    ...
    {
        $id = Mage::objects()->save($user);
        Mage::register('Object_Cache_User', $id);
    }

    public function getUser()
    {
        return Mage::objects()->load(Mage::registry('Object_Cache_User'));
    }
    ...
    public function save()
    {
        ...
        if ($collections instanceof Mage_User_Model_Entity_User_Collection) {
            $collections = array($collections->getEntity()->getStoreId()=>$collections);
        } elseif (!is_array($collections)) {
            $this->addException(Mage::helper('user')->__('No user collections found'), Mage_Dataflow_Model_Convert_Exception::FATAL);
        }

        foreach ($collections as $storeId=>$collection) {
            $this->addException(Mage::helper('user')->__('Records for %s store found.', $stores[$storeId]));

            if (!$collection instanceof Mage_User_Model_Entity_User_Collection) {
                $this->addException(Mage::helper('user')->__('User collection expected.'), Mage_Dataflow_Model_Convert_Exception::FATAL);
            }
            try {
                $i = 0;
                foreach ($collection->getIterator() as $model) {
                    $new = false;
                    // if user is new, create default values first
                    if (!$model->getId()) {
                        $new = true;
                        $model->save();
...
//                        }
                ...
                $this->addException(Mage::helper('user')->__("Saved %d record(s)", $i));
            } catch (Exception $e) {
                if (!$e instanceof Mage_Dataflow_Model_Convert_Exception) {
                    $this->addException(Mage::helper('user')->__('An error occurred while saving the collection, aborting. Error: %s', $e->getMessage()),
    ...
    /*
     * saveRow function for saving each user data
     *
     * params args array
     * return array
     */
    public function saveRow($importData)
    {
        ...
        if (empty($importData['website'])) {
            $message = Mage::helper('user')->__('Skipping import row, required field "%s" is not defined.', 'website');
        ...
        if ($website === false) {
            $message = Mage::helper('user')->__('Skipping import row, website "%s" field does not exist.', $importData['website']);
            Mage::throwException($message);
        }
        if (empty($importData['email'])) {
            $message = Mage::helper('user')->__('Skipping import row, required field "%s" is not defined.', 'email');
            Mage::throwException($message);
        }

        $user->setWebsiteId($website->getId())
        ...
        if (!$user->getId()) {
             ...
             * Check user group
             */
            if (empty($importData['group']) || !isset($userGroups[$importData['group']])) {
                $value = isset($importData['group']) ? $importData['group'] : '';
                $message = Mage::helper('adminhtml')->__('Skipping import row, the value "%s" is not valid for the "%s" field.', $value, 'group');
                Mage::throwException($message);
            }
            $user->setGroupId($userGroups[$importData['group']]);

            foreach ($this->_requiredFields as $field) {
                if (!isset($importData[$field])) {
                    $message = Mage::helper('adminhtml')->__('Skip import row, required field "%s" for the new user is not defined.', $field);
                    Mage::throwException($message);
                }
            }
            ...
            if (empty($importData['created_in']) || !$this->getStoreByCode($importData['created_in'])) {
                ...
                $user->setStoreId($this->getStoreByCode($importData['created_in'])->getId());
            }

            if (empty($importData['password_hash'])) {
                $user->setPasswordHash($user->hashPassword($user->generatePassword(8)));
            }
        }
        elseif (!empty($importData['group'])) {
             ...
             * Check user group
             */
            if (isset($userGroups[$importData['group']])) {
                $user->setGroupId($userGroups[$importData['group']]);
            }
        }
        ...
        foreach ($importData as $field => $value) {
            ...
            $user->setData($field, $setValue);
        }

        if (isset($importData['is_subscribed'])) {
            $user->setData('is_subscribed', $importData['is_subscribed']);
        }
        ...
        if ($importBillingAddress) {
            ...
            if ($user->getDefaultBilling()) {
                $billingAddress->load($user->getDefaultBilling());
            }
            else {
            ...
            if ($user->getId()) {
                ...
                $billingAddress->save();
                $user->setDefaultBilling($billingAddress->getId());

                if ($onlyAddress) {
                    $user->setDefaultShipping($billingAddress->getId());
                }
        ...
        if ($importShippingAddress) {
            ...
            if ($user->getDefaultShipping() && $user->getDefaultBilling() != $user->getDefaultShipping()) {
                $shippingAddress->load($user->getDefaultShipping());
            }
            else {
            ...
            if ($user->getId()) {
                ...
                $user->setDefaultShipping($shippingAddress->getId());

                $savedShippingAddress = true;
            }
        }

        $user->setImportMode(true);
        $user->save();
        $saveUser = false;
    ...
    public function saveRow__OLD()
    {
        ...
        try {
            ...
            $user->unsetData();
            $user->cleanAllAddresses();
            $user->unsetSubscription();
            $newMem = memory_get_usage(); $memory .= ', '.($newMem-$mem); $mem = $newMem;
