...
Moneybookers.prototype = {
    initialize: function(bannerUrl, activateemailUrl, checksecretUrl, checkemailUrl){
        ...
        this.txtInfStatus0 = this.translate('<strong><a href="http://www.moneybookers.com/" target="_blank">Moneybookers</a></strong> is an all in one payments solution that enables a merchant to accept debit and credit card payments, bank transfers and the largest range of local payments directly on your website.<ul style="list-style-position: inside;list-style-type: disc;"><li>Widest network of international and local payment options in the world.</li><li>One interface including payments, banking and marketing.</li><li>Direct payments without the need for direct registration.</li><li>Moneybookers stands for a highly converting payment gateway that turns payment processing into a simple, fast and user friendly operation.</li><li>Highly competitive rates. Please <a href="http://www.moneybookers.com/app/help.pl?s=m_fees" target="_blank">click here</a> for more detailed information.</li></ul>') + '<img src="' + this.bannerUrl + '" alt="" />';
        this.txtInfStatus1 = this.translate('<strong>Moneybookers Quick Checkout</strong> enables you to take <strong>direct</strong> payments from credit cards, debit cards and over 50 other local payment options in over 200 countries for users without an existing Moneybookers eWallet.');
        ...
        $("moneybookers_settings_moneybookers_email").setAttribute("onchange", "moneybookers.setStatus(0); moneybookers.changeUi(); document.getElementById('moneybookers_settings_user_id').value = ''; document.getElementById('moneybookers_settings_user_id_hidden').value = '';");
        $("moneybookers_settings_user_id").disabled = true;
        $("moneybookers_settings_user_id_hidden").name = document.getElementById("moneybookers_settings_user_id").name;
        $("moneybookers_settings_user_id_hidden").value = document.getElementById("moneybookers_settings_user_id").value;
        ...
        if (this.isStoreView()) {
            this.infoOrig = {
                ...
                userId: $("moneybookers_settings_user_id").value,
                key: $("moneybookers_settings_secret_key").value,
                status: this.getStatus(),
                useDefult: $("moneybookers_settings_moneybookers_email_inherit").checked
            };
            var defaults = $$("#row_moneybookers_settings_user_id .use-default, #row_moneybookers_settings_secret_key .use-default, #row_moneybookers_settings_activationstatus .use-default");
    ...
    button: function () {
        ...
        if (status < 1) {
            ...
            if (result[0] == "OK") {
                $("moneybookers_settings_user_id").value = result[1];
                $("moneybookers_settings_user_id_hidden").value = result[1];
                this.setStatus(1);
                status = 1;
                alert(this.txtNotStatus0);
            }
            else {
                $("moneybookers_settings_user_id").value = "";
        ...
        if (status == 2) {
            response = this.getHttp(this.checksecretUrl + "?email=" + $("moneybookers_settings_moneybookers_email").value
                ...
                + "&cust_id=" + $("moneybookers_settings_user_id").value);
            if (response == "OK") {
                this.setStatus(3);
    ...
    getStatus: function () {
        ...
        if (status == 1 && $("moneybookers_settings_user_id").value != '' && $("moneybookers_settings_secret_key").value == '') {
            status = 2;
    ...
    changeStore: function () {
        if (!$("moneybookers_settings_moneybookers_email_inherit").checked) {
            if (this.infoOrig.useDefult) {
                $("moneybookers_settings_user_id_inherit").click();
                $("moneybookers_settings_user_id").clear();
            ...
            if (this.infoOrig.useDefult) {
                $("moneybookers_settings_user_id").setValue(this.infoOrig.userId);
                $("moneybookers_settings_user_id_inherit").click();
